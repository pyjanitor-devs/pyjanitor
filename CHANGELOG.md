...
## [Unreleased]
-   [PERF] Use an O(n + m) NumPy prefix/suffix running-argmin/argmax for
    forward `conditional_join` range `min`/`max` aggregations
    (`join_agg(..., aggfunc=[(col, "min"|"max")])`), replacing the O(sum
