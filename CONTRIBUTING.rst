...
.. code-block:: bash
   :emphasize-lines: 8
    ...
    │   └── test_read_csvs.py
    ├── spark
    |   ├── conftest.py  # contains pyspark setup and pyspark test dataframes
    │   └── functions
...
When you run the test locally, the tests in ``chemistry.py``, ``biology.py``, ``janitor.spark`` are automatically skipped if you don't have the optional dependencies (e.g. ``rdkit``) installed.
