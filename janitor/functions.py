...
@pf.register_dataframe_accessor("string")
class StringAccessor:
    """
    Single Wrapper to access Pandas' string methods.
    String function should return dataframe with modified column.
    """

    def __init__(self, df):
        self._df = df

    def concat(self, column, others=None, sep=None, na_rep=None, join="left"):
        """
        Concatenate strings in a column, or with ``others``,
        using the given separator
        """
        df = self._df
        df.loc[:, column] = df.loc[:, column].str.cat(
            others, sep, na_rep, join
        )
        return df

    def split(self, column, pat=None, n=-1):
        """
        Split strings around given delimiter.
        Expand parameter will always be False,
        to ensure only the target column is modified
        """
        df = self._df
        df.loc[:, column] = df.loc[:, column].str.split(pat, n, expand=False)
        return df

    def rsplit(self, column, pat=None, n=-1):
        """
        Split strings from the end around given delimiter.
        Expand parameter will always be False
        to ensure only the target column is modified"""
        df = self._df
        df.loc[:, column] = df.loc[:, column].str.rsplit(pat, n, expand=False)
        return df

    def get(self, column, i):
        """
        Extract element from string/list/tuple
        at specified position
        """
        df = self._df
        df.loc[:, column] = df.loc[:, column].str.get(i)
        return df

    def join(self, column, sep):
        """
        Join lists contained as elements in column,
        with passed separator/delimiter
        """
        df = self._df
        df.loc[:, column] = df.loc[:, column].str.join(sep)
        return df
