...
def _agg_join_left(df: pd.DataFrame, aggfunc: list, indices: dict) -> pd.DataFrame:
            ...
            if agg == "size":
                ...
                if agg in {
                    ...
                    if bools.any():
                        out = out.mask(bools)
                    out = out._values
                # print('agg', agg, 'out', out, 'index', _index, 'arr', arr)
    ...
    # for many-to-one, or one-to-one
        ...
        for column_name, agg in aggfunc:
            if agg == "size":
                out = pd.Series(right_index).value_counts(sort=False)
            else:
                ser = df[column_name]
                arr = ser._values
                booleans = pd.isna(arr)
        ...
        length = indices["counts_array"].sum()
        for column_name, agg in aggfunc:
            if agg == "size":
                func = mapping[agg]
                _index, out = func(
        ...
        length = indices["ends"] - indices["starts"]
        length = length.sum()
        for column_name, agg in aggfunc:
            if agg == "size":
                func = mapping[agg]
