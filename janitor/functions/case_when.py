...
def case_when(df: pd.Series, *args: Any, default: Any = None) -> pd.Series:
    """Create a pandas Series based on a condition or multiple conditions.
    ...
    Args:
        ...
        *args: Variable argument of conditions and expected values.
            ...
            `condition` can be a 1-D boolean array or a callable.
            If `condition` is a callable, it should evaluate
            to a 1-D boolean array. The array should have the same length
            as the DataFrame. `result` can be a scalar, a 1-D array,
            or a callable. If `result` is a callable,
            it should evaluate to a 1-D array. For a 1-D array,
            it should have the same length as the DataFrame.
        default: This is the element inserted in the output
            ...
            The 1-D array should be the same length as the Series.

    Returns:
        A pandas Series.
    """
    booleans, replacements, default = _case_when_checks(
        df=df, column_name=None, args=args, default=default
    )
    return _compute_case_when(
        booleans=booleans, replacements=replacements, default=default
    )


@pf.register_dataframe_method
def case_when(  # noqa: F811
    ...
    """  # noqa: E501
    booleans, replacements, default = _case_when_checks(
        df=df, column_name=column_name, args=args, default=default
    )
    default = _compute_case_when(
        booleans=booleans, replacements=replacements, default=default
    )
...
def _case_when_checks(
    ...
    """
    df_is_a_dataframe = isinstance(df, pd.DataFrame)
    if df_is_a_dataframe:
    ...
    if len_args % 2:
        if df_is_a_dataframe and (default is None):
            warnings.warn(
    ...
    for index, value in enumerate(args):
        if index % 2:
            ...
            if callable(value):
                value = apply_if_callable(value, df)
            elif df_is_a_dataframe and isinstance(value, str):
                value = df.eval(value)
    ...
    if is_scalar(default):
        default = pd.array([default]).repeat(len(df))
...
def _compute_case_when(
    booleans: list,
    replacements: list,
    default: pd.Series,
) -> pd.Series:
    """
    Computes the Series based on the conditions.
    """
    # actual computation
