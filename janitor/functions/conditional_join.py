...
def _conditional_join_compute(
    ...
    # is an integer tape indexing ``right_index``. Both frames were normalized
    # to ``range(len(frame))`` above, so ``left_index`` and ``right_index`` hold
    # row positions rather than the caller's index labels. Depending on join
    # shape, some representations are absent: equality joins may return pairs
    # directly, while range joins commonly retain boundaries until aggregation.
    # Empty ranges have equal boundaries and contribute no matches.

    # For example, with ``right_index = [0, 1, 2, 3]``,
    # ``positions = [2, 0, 2, 3, 1]``, ``starts = [0, 2, 4]`` and
    # ``ends = [2, 4, 5]``, the three driving rows select right positions
    # ``[2, 0]``, ``[2, 3]`` and ``[1]`` respectively. Simple/equi joins
    # generally return direct pairs; starts-only/ends-only paths represent
    # one-sided inequalities; range and multi-condition joins may retain both
    # boundaries and a mask. ``keep="first"`` or ``keep="last"`` reduces each
    # slice before the pairs are emitted, while ``keep="all"`` emits every
    # surviving position.
