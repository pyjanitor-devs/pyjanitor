...
def _data_checks_pivot_longer(
    ...
    if (names_sep is None) and (names_pattern is None) and (index is not None):
        if values_to in index:
            raise ValueError(
...
def _computations_pivot_longer(
    ...
    if column_names.size != column_names.nunique():
