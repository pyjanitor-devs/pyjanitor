...
def _sub_expand_grid(value, grid_index, key):  # noqa: F811
    ...
    return {(key, 0): value.take(grid_index, axis=0)}
...
def _sub_expand_grid(value, grid_index, key):  # noqa: F811
    ...
    if not name:
        name = 0
    value = extract_array(value, extract_numpy=True).take(grid_index, axis=0)
...
def _sub_expand_grid(value, grid_index, key):  # noqa: F811
    ...
    return {
        (key, name): extract_array(val, extract_numpy=True).take(
            grid_index, axis=0
        )
...
def _sub_expand_grid(value, grid_index, key):  # noqa: F811
    ...
    for n in range(value.nlevels):
        ...
        name = arr.name
        arr = extract_array(arr, extract_numpy=True).take(grid_index, axis=0)
        if not name:
            name = num
            num += 1
...
def _sub_expand_grid(value, grid_index, key):  # noqa: F811
    ...
    if not name:
        name = 0
    return {
        (key, name): extract_array(value, extract_numpy=True).take(
            grid_index, axis=0
        )
    }
