...
def test_length_conversion(dataframe):
    df = dataframe.convert_units(
        ...
        existing_units='cm',
        to_units='m',
        dest_column_name='a_m'
    )
    assert df['a'].sum() == pytest.approx(df['a_m'].sum() * 100.)
...
def test_mass_conversion(dataframe):
    df = dataframe.convert_units(
        ...
        existing_units='g',
        to_units='kg',
        dest_column_name='a_kg'
    )
    assert df['a'].sum() == pytest.approx(df['a_kg'].sum() * 1000.)
...
def test_area_conversion(dataframe):
    df = dataframe.convert_units(
        ...
        existing_units='cm**2',
        to_units='m**2',
        dest_column_name='a_m2'
    )
    assert df['a'].sum() == pytest.approx(df['a_m2'].sum() * (100. ** 2))
...
def test_volume_conversion(dataframe):
    df = dataframe.convert_units(
        ...
        existing_units='cm**3',
        to_units='m**3',
        dest_column_name='a_m3'
    )
    assert df['a'].sum() == pytest.approx(df['a_m3'].sum() * (100. ** 3))