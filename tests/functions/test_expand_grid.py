...
def expand_data(entry):
    ...
    if not isinstance(entry, dict):
        raise ValueError("expand_grid works with only name value pairs")

    dfs, dicts = check_instance(entry)
...
def compute_two_dfs(df1, df2):
    ...
    columns = chain.from_iterable(ent.columns for ent in (df1, df2))
...
def test_dataframe_multiIndex_column():
    data = {
        "x":
        pd.DataFrame([[2, 3], [6, 7]],
