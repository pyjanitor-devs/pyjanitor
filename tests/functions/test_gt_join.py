...
def test_df_MultiIndex(multiIndex_df, right_df):
    ...
    with pytest.raises(
        ...
        multiIndex_df.gt_join(right_df, "col_a", "col_a")
...
def test_wrong_length_suffixes(left_df, right_df):
    ...
    with pytest.raises(
        ...
        left_df.gt_join(right_df, "col_a", "col_a", suffixes=("_x",))
...
def test_suffixes_None(left_df, right_df):
    ...
    with pytest.raises(
        ...
        left_df.gt_join(right_df, "col_a", "col_a", suffixes=(None, None))
...
def test_wrong_type_suffix(left_df, right_df):
    ...
    """
    with pytest.raises(TypeError):
        left_df.gt_join(right_df, "col_a", "col_a", suffixes=("_x", 1))
...
def test_suffix_already_exists_df(left_df, right_df):
    ...
    left_df["col_a_x"] = 2
    with pytest.raises(ValueError):
        left_df.gt_join(right_df, "col_a", "col_a")
...
def test_column_same_type(left_df, right_df):
    ...
    with pytest.raises(ValueError):
        left_df.gt_join(right_df, "col_a", "col_c")
...
various = [
    (
        ...
        "col_a",
        False,
        pd.DataFrame([], columns=("col_a_x", "col_b", "col_a_y", "col_c")),
    ),
    (
        pd.DataFrame({"col_a": [1, 2, 3], "col_b": ["A", "B", "C"]}),
        pd.DataFrame({"col_a": [4, 6, 7.5], "col_c": ["Z", "X", "Y"]}),
...
def test_various_scenarios(
    ...
    """Test various scenarios for gt_join"""
    expected = left_df.gt_join(
